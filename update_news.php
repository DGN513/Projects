...
// Получаем ID новости из GET-запроса
if (isset($_GET['id'])) {
    $newsId = intval($_GET['id']);

    // Получаем данные новости для заполнения формы
    $newsQuery = $conn->prepare("SELECT title, description, id_category FROM News WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($title) && !empty($description) && !empty($id_category)) {
        // Обновляем новость
        $sql_update = "UPDATE `News` SET `title` = ?, `description` = ?, `id_category` = ? WHERE `id` = ?";
        $stmt = $conn->prepare($sql_update);
        $stmt->bind_param("ssii", $title, $description, $id_category, $newsId);
        ...
        if ($stmt->execute()) {
            ...
            echo "Ошибка обновления новости: " . mysqli_error($conn);
        }
    } else {
        ...
        <div class="align_box_f">
            <div class="box">
                <form class="box_form" method="post">
                    <p class="regist">Обновить новость</p>

                    <div class="mb-3">
                        <label for="title" class="form-label">Заголовок</label>
                        <input type="text" name="title" class="form-control" id="title" value="<?= htmlspecialchars($news['title']) ?>" required>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label">Описание новости</label>
                        <textarea name="description" class="form-control" id="description" rows="3" required><?= htmlspecialchars($news['description']) ?></textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="id_category" class="form-select" id="category" required>
                            ...
                            <?php foreach ($services as $service): ?>
                                ...
                                <?= $service['id_category'] == $news['id_category'] ? 'selected' : '' ?>><?= $service['name_category'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
